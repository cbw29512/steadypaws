...
def assert_catalog() -> None:
    ...
    if len(TRACKERS) != 72:
        raise AssertionError(f"Expected 72 care forms, found {len(TRACKERS)}")
    ...
    LOGGER.info("Shared 72-form catalog: PASS")
...
def assert_homepage() -> None:
    ...
    if parser.tracker_cards != len(TRACKERS):
        raise AssertionError(f"Expected {len(TRACKERS)} care cards, found {parser.tracker_cards}")
    ...
    for item in TRACKERS:
        ...
        if expected not in html:
            raise AssertionError(f"Homepage missing care-form link: {expected}")
...
def assert_pdfs() -> None:
    for item in TRACKERS:
        ...
        if "Their name" not in first_page:
            raise AssertionError(f"Family-first identity label missing: {item['filename']}")
        if "How they did this week" not in second_page:
            raise AssertionError(f"Family-first weekly wording missing: {item['filename']}")
    LOGGER.info("All 72 family-first printable PDFs (144 pages): PASS")
